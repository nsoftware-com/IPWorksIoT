...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, iotcore, iottypes, iotjson, StrUtils;
...
type
  TFormJson = class(TForm)
    ...
    procedure iotJSON1EndElement(Sender: TObject; const Element: string);
    procedure iotJSON1StartElement(Sender: TObject; const Element: string);
...
begin
  Try
    ...
    if(rbtnFile.Checked) Then
      iotJSON1.InputFile := txtFile.Text;
    begin
      inputString := '';
      for I := 0 to memoString.Lines.Count -1 do
        ...
        iotJSON1.InputData := inputString;
    end;

    iotJSON1.Parse();
    iotJSON1.XPath := '/';
    root := iotJSON1.XElement;
    iotJSON1.XPath := '/' + root;
    tvwXPath.Items.AddFirst(nil, iotJSON1.XPath);
    numChildren := iotJSON1.XChildCount;

    for I := 1 to numChildren do
    begin
      iotJSON1.XPath := '/' + root + '/[' + IntToStr(I) + ']';
      try
          elemName := iotJSON1.XElement;
...
procedure TFormJson.iotJSON1EndElement(Sender: TObject; const Element: string);
begin
  memoEvents.Lines.Add('End Element: ' + Element);
end;


procedure TFormJson.iotJSON1StartElement(Sender: TObject; const Element: string);
begin
...
begin
  ...
  if tvwXPath.Selected <> nil then
    ...
    iotJSON1.XPath := nodePath;
    txtJPath.Text := iotJSON1.XPath;
    txtElement.Text := iotJSON1.XElement;
    txtValue.Text := iotJSON1.XText;
    ...
    for I := 1 to numChildren do
      begin
        iotJSON1.XPath := parent + '/[' + IntToStr(I) + ']';
        try
          elemName := iotJSON1.XElement;
          if(Length(elemName) = 0) Then
            elemName := '[' + IntToStr(I) + ']';
